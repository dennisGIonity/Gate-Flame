...
| ~~`sudo` on the Pi needs a password~~ | **Corrected 2026-08-31: it does not.** `sudo -n true` succeeds as `wabapi` — sudo is passwordless. Staging a script and asking Dennis to run it is still the right habit for anything that touches the household (RAs, gateway claim, the resolver), but it is a *judgement* call, not a technical limit. `wabapi` is also in the `docker` group, so `docker`/`docker exec` work unprivileged. |
