...
const handleWhitelistSubmit = (e: React.FormEvent) => {
  ...
  const handlePiholeToggle = async (action: 'enable' | 'disable', seconds?: number) => {
    try {
      const baseUrl = userAccount?.nodeIpAddress ? userAccount.nodeIpAddress.replace(/\/$/, '') : 'http://localhost:3001';
      const opts = { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action, seconds }) };
      await fetch(`${baseUrl}/api/pihole/toggle`, opts).catch(() => fetch('/api/pihole/toggle', opts));
    } catch (e) {
      console.warn('Failed to toggle Pi-hole', e);
    }
  };
                    ...
                    >
                        <GlassPanel filterLevel={telemetry.filterLevel} className="rounded-3xl p-5 space-y-6">
                            ...
                            {telemetry.protectionStatus === 'active' ? (
                                ...
                                <button onClick={() => { onPauseProtection(5); handlePiholeToggle('disable', 5 * 60); }} className="bg-sky-100 dark:bg-white/5 hover:bg-rose-500/10 text-rose-500 dark:text-rose-400 border border-sky-200 dark:border-white/10 py-3 rounded-xl text-[10px] font-medium transition-all">
                                    5m
                                </button>
                                <button onClick={() => { onPauseProtection(15); handlePiholeToggle('disable', 15 * 60); }} className="bg-sky-100 dark:bg-white/5 hover:bg-amber-500/10 text-amber-500 dark:text-amber-400 border border-sky-200 dark:border-white/10 py-3 rounded-xl text-[10px] font-medium transition-all">
                                    15m
                                </button>
                                <button onClick={() => { onPauseProtection(60); handlePiholeToggle('disable', 60 * 60); }} className="bg-sky-100 dark:bg-white/5 hover:bg-slate-300 dark:hover:bg-white/10 text-sky-900 dark:text-slate-300 border border-sky-200 dark:border-white/10 py-3 rounded-xl text-[10px] font-medium transition-all">
                                    1h
                                </button>
                                </div>
                            ) : (
                                <button onClick={() => { onResumeProtection(); handlePiholeToggle('enable'); }} className="w-full bg-emerald-500 text-white dark:text-black py-3 rounded-xl text-sm font-bold transition-all shadow-[0_0_15px_rgba(16,185,129,0.3)]">
                                RESUME
                                </button>
                            )}
                            </div>
