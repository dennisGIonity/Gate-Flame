...
export const ApiService = {
  ...
  toggleService: async (moduleId: string, endpoint: string, enable: boolean): Promise<boolean> => {
    try {
      ...
      const ipAddress = useAppStore.getState().userAccount?.nodeIpAddress;
      const baseUrl = ipAddress ? ipAddress.replace(/\/$/, '') : 'http://localhost:3001';
      ...
      if (!response || !response.ok) throw new Error('Hardware API rejected request');
      
      // Simulating network/hardware spin-up delay for UX
