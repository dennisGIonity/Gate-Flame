...
let code = `import React, { useEffect, useRef, useState } from 'react';
...
export const IonicrobesGame: React.FC<{ embedded?: boolean }> = ({ embedded = false }) => {
  ...
  return (
    <div className={\`relative w-full \${embedded ? "h-full" : "h-[400px]"} bg-slate-950/80 rounded-2xl border border-slate-800 overflow-hidden font-sans flex flex-col shadow-inner\`}>
      {/* Header */}
